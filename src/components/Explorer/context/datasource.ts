...
export function EthersDataSource(provider: ethers.providers.Provider): DataSource {
    const ERC245Contract = new ethers.Contract(SupplyChainAddress, ERC245.abi, provider)
    const ERC423Contract = new ethers.Contract(SupplyChainAddress, ERC423.abi, provider)
    return {
        erc245: {
            getAsset: async (id) => {
